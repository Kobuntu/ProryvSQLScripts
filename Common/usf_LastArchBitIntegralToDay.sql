if exists (select 1
          from sysobjects
          where  id = object_id('usf2_LastArchBitIntegralToDay')
          and type in ('IF', 'FN', 'TF'))
   drop function usf2_LastArchBitIntegralToDay
go
...
-- Описание:
--
-- Формируем список значений по суткам из интегральных данных
-- ======================================================================================
create FUNCTION [dbo].[usf2_LastArchBitIntegralToDay] (
...
insert into @tbl
SELECT @TI_ID, PivotTable.EventDate, @ChannelType, 
...
from usf2_LastArchBitIntegralToHH (@TI_ID, @ChannelType, @EventDateTime, @VAL, @PrevDateTime, @PrevValue)
PIVOT
(
			--Берем среднее (на тот случай если будет несколько данных за одно и тоже время)
			avg([VAL]) FOR [EventDateID] IN ([0],[30],[60],[90],[120],[150],[180],[210],[240],[270],[300],[330],[360],[390],[420],[450],[480],[510],[540],[570],[600],[630],[660],[690],[720],[750],[780],[810],[840],[870],[900],[930],[960],[990],[1020],[1050],[1080],[1110],[1140],[1170],[1200],[1230],[1260],[1290],[1320],[1350],[1380],[1410])
) AS PivotTable
...
END
go
grant select on usf2_LastArchBitIntegralToDay to [UserCalcService]
go
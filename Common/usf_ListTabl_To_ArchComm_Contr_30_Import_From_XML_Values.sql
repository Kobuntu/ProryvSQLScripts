if exists (select 1
          from sysobjects
          where  id = object_id('usf2_ListTabl_To_ArchComm_Contr_30_Import_From_XML_Values')
          and type in ('IF', 'FN', 'TF'))
   drop function usf2_ListTabl_To_ArchComm_Contr_30_Import_From_XML_Values
go
...
--		в местах где отсутствует значение пишем NULL, поэтому при обновлении надо проверять на NULL
--		эта прроцедура для КА
-- ======================================================================================
create FUNCTION [dbo].[usf2_ListTabl_To_ArchComm_Contr_30_Import_From_XML_Values] (
...
END
go
grant select on usf2_ListTabl_To_ArchComm_Contr_30_Import_From_XML_Values to [UserCalcService]
go

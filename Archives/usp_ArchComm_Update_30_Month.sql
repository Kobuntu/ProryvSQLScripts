if exists (select 1
          from sysobjects
          where  id = object_id('usp2_ArchComm_Update_30_Month')
          and type in ('P','PC'))
   drop procedure usp2_ArchComm_Update_30_Month
go
...
-- Описание:
--
--		Импорт 30 минутных значений в таблицу малых ТИ
--
-- ======================================================================================

create proc [dbo].[usp2_ArchComm_Update_30_Month]
	@TI_Array varchar(4000),-- Идентификатор ТИ (строго 10 цифр) , время (строго 16 цифр), значение, номер канала между ними , в конце обязательна ;
	@DispatchDateTime DateTime, --Время когда запись была вставлена
	@EventParam tinyint --Признак замещения (0- неопределено,1- ручной ввод,2- КА,3-Данные измерений, 4- Распределение 0 на определенный период)
...
begin
	...
	from 
				usf2_ListTabl_To_ArchComm_Update_30_Month_Values(@TI_Array) a
				left join 
				usf2_ListTabl_To_ArchComm_Update_30_Month_Validate(@TI_Array) v
...
----Обновляем таблицу архивных значений-------------------------------------
		...
		CUS_ID = @CUS_ID,
		ValidStatus = (ArchCalc_30_Month_Values.ValidStatus & (~NotNullStatus)) | fl.ValidStatus
		from 
		(
			select * from #TmpResult where not [oldday] is null
		) fl
		where ArchCalc_30_Month_Values.TI_ID=fl.TI_ID and ArchCalc_30_Month_Values.ChannelType=fl.ChannelType and ArchCalc_30_Month_Values.EventDate = fl.CalendarDayDate

	---Добавляем поля которых нет 
		insert	ArchCalc_30_Month_Values
		(
			TI_ID,EventDate,ChannelType,[VAL_01],[VAL_02],[VAL_03]
			  ,[VAL_04]
			  ,[VAL_05]
			  ,[VAL_06]
			  ,[VAL_07]
...
go
   grant EXECUTE on usp2_ArchComm_Update_30_Month to [UserCalcService]
go
if exists (select 1
          from sysobjects
          where  id = object_id('usp2_Info_GetPSParams')
          and type in ('P','PC'))
   drop procedure usp2_Info_GetPSParams
go
SET ANSI_NULLS ON
GO
...
--
-- ======================================================================================
create proc [dbo].[usp2_Info_GetPSParams]
(	
	@PS_ID_List varchar(4000),	
	@datestart datetime,
	@dateend datetime
)
...
begin
	...
	--Перебираем все ПС
	declare t cursor LOCAL STATIC FORWARD_ONLY READ_ONLY for select Cast(STRnumber as int) as PS_ID from usf2_Utils_iter_strlist_to_table(@PS_ID_List)
	open t;
	FETCH NEXT FROM t into @PS_ID
	WHILE @@FETCH_STATUS = 0
	BEGIN
		set @BalancePS_UN = dbo.usf2_Info_GetFirstPSBalance (@PS_ID);

		select @BalancePS_UN as BalancePS_UN, (select top 1 PSType from Dict_PS where PS_ID = @PS_ID) as PSType;
...
go
   grant exec on usp2_Info_GetPSParams to [UserCalcService]
go
if exists (select 1
          from sysobjects
          where  id = object_id('usf2_Info_GetFormulasNeededForMainFormulaContr2')
          and type in ('IF', 'FN', 'TF'))
   drop function usf2_Info_GetFormulasNeededForMainFormulaContr2
go
...
create FUNCTION [dbo].[usf2_Info_GetFormulasNeededForMainFormulaContr2]
(	
...
begin
	...
	(
		...
		from	
			(
				select UsedFormula_UN from dbo.Info_TP2_Contr_Formula_Description 
				where Formula_UN = @formulaId and not UsedFormula_UN is null
			) uifd
		inner join 
			dbo.Info_TP2_Contr_Formula_List uifl on  uifl.Formula_UN = uifd.UsedFormula_UN
		left join 
			dbo.Info_TP2_Contr_Formula_Description uifd2 on uifl.Formula_UN = uifd2.Formula_UN
		...
		from FormulasParams
		inner join
			dbo.Info_TP2_Contr_Formula_List uifl on  uifl.Formula_UN = FormulasParams.UsedFormula_UN
		inner join 
			dbo.Info_TP2_Contr_Formula_Description uifd2 on uifl.Formula_UN = uifd2.Formula_UN
	)
...
go
   grant select on usf2_Info_GetFormulasNeededForMainFormulaContr2 to [UserCalcService]
go